...
describe("AncientRunicLeatherHorseMartingaleBenchEngine Equestrian Martingale Stitching Benches & Rigs", () => {
    it("crafts Celestial Void Valkyrie Sovereign Martingale in Martingale Sanctum achieving 100% anchor stability and returns remaining pelts", () => {
        const bench = AncientRunicLeatherHorseMartingaleBenchEngine.constructBench("leather_01", "CELESTIAL_VOID_VALKYRIE_MARTINGALE_SANCTUM");
        expect(bench.benchType).toBe("CELESTIAL_VOID_VALKYRIE_MARTINGALE_SANCTUM");
        ...
        expect(craftRes.martingale?.martingaleAnchorStabilityPercent).toBe(100);
        expect(craftRes.martingale?.finalReinTensionStabilityPercent).toBe(100); // 84 * 1.20 = 100.8 -> clamped to 100%
        expect(craftRes.martingale?.finalHeadTossPreventionPercent).toBe(77); // 64 * 1.20 = 76.8 -> 77%
    ...
    it("verifies mid-range stability roll and sub-100% quality scaling on Elder martingale bench", () => {
        const bench = AncientRunicLeatherHorseMartingaleBenchEngine.constructBench("leather_mid", "ELDER_MARTINGALE_BENCH");
        ...
        // finalTossPrevention = Math.round(14 * 0.944) = 13
        const craftRes = AncientRunicLeatherHorseMartingaleBenchEngine.craftMartingale(
            bench,
            "NOVICE_EXPEDITION_MARTINGALE",
            ["TANNED_CENTAUR_MARTINGALE_STRAP", "TANNED_CENTAUR_MARTINGALE_STRAP"],
            0.1,
            0.5
        );

        expect(craftRes.success).toBe(true);
        expect(craftRes.martingale?.martingaleAnchorStabilityPercent).toBe(36);
        expect(craftRes.martingale?.finalReinTensionStabilityPercent).toBe(23);
        expect(craftRes.martingale?.finalHeadTossPreventionPercent).toBe(13);
    });

    it("handles bench becoming non-functional after successful craft when durability falls below threshold", () => {
        const bench = AncientRunicLeatherHorseMartingaleBenchEngine.constructBench("leather_wear", "ELDER_MARTINGALE_BENCH");
    ...
    it("rejects crafting when insufficient leather is provided and returns provided leathers", () => {
        const bench = AncientRunicLeatherHorseMartingaleBenchEngine.constructBench("leather_02", "ELDER_MARTINGALE_BENCH");

        const failRes = AncientRunicLeatherHorseMartingaleBenchEngine.craftMartingale(
            bench,
            "WARMASTER_MITHRIL_REINFORCED_MARTINGALE",
            ["TEMPERED_MITHRIL_MARTINGALE_FORK_RING"]
        );

        expect(failRes.success).toBe(false);
        expect(failRes.reason).toContain("Insufficient martingale straps/fork rings");
        expect(failRes.remainingProvidedLeathers.length).toBe(1);
        expect(bench.currentDurability).toBe(95);
    });

    it("handles martingale strap misaligned failure roll consuming durability and leathers", () => {
        const bench = AncientRunicLeatherHorseMartingaleBenchEngine.constructBench("leather_03", "ELDER_MARTINGALE_BENCH"); // 87% success

        const fail = AncientRunicLeatherHorseMartingaleBenchEngine.craftMartingale(
            bench,
            "NOVICE_EXPEDITION_MARTINGALE",
            ["TANNED_CENTAUR_MARTINGALE_STRAP", "TANNED_CENTAUR_MARTINGALE_STRAP", "TANNED_CENTAUR_MARTINGALE_STRAP"],
            0.95
        );
    ...
    it("gates isFunctional in maintainBench based on DURABILITY_COST_PER_CRAFT threshold and returns clone", () => {
        const bench = AncientRunicLeatherHorseMartingaleBenchEngine.constructBench("leather_04", "ELDER_MARTINGALE_BENCH");
    ...
    it("guards against null inputs and unsupported bench models", () => {
        ...
        expect(AncientRunicLeatherHorseMartingaleBenchEngine.craftMartingale(invalidBench, "NOVICE_EXPEDITION_MARTINGALE", ["TANNED_CENTAUR_MARTINGALE_STRAP", "TANNED_CENTAUR_MARTINGALE_STRAP"]).success).toBe(false);
        expect(AncientRunicLeatherHorseMartingaleBenchEngine.craftMartingale(null as any, "NOVICE_EXPEDITION_MARTINGALE", []).success).toBe(false);
