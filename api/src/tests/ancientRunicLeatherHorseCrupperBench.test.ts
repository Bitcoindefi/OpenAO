...
describe("AncientRunicLeatherHorseCrupperBenchEngine Equestrian Crupper Stitching Benches & Rigs", () => {
    it("crafts Celestial Void Valkyrie Sovereign Crupper in Crupper Sanctum achieving 100% anchor stability and returns remaining pelts", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_01", "CELESTIAL_VOID_VALKYRIE_CRUPPER_SANCTUM");
        expect(bench.benchType).toBe("CELESTIAL_VOID_VALKYRIE_CRUPPER_SANCTUM");
        ...
        expect(craftRes.crupper?.crupperAnchorStabilityPercent).toBe(100);
        expect(craftRes.crupper?.finalDownhillSaddleRetentionPercent).toBe(100); // 84 * 1.20 = 100.8 -> clamped to 100%
        expect(craftRes.crupper?.finalTailDockComfortPercent).toBe(77); // 64 * 1.20 = 76.8 -> 77%
    ...
    it("verifies mid-range stability roll and sub-100% quality scaling on Elder crupper bench", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_mid", "ELDER_CRUPPER_BENCH");
        ...
        const craftRes = AncientRunicLeatherHorseCrupperBenchEngine.craftCrupper(
            bench,
            "NOVICE_EXPEDITION_CRUPPER",
            ["TANNED_STALLION_CRUPPER_STRAP", "TANNED_STALLION_CRUPPER_STRAP"],
            0.1,
            0.5
        );

        expect(craftRes.success).toBe(true);
        expect(craftRes.crupper?.crupperAnchorStabilityPercent).toBe(36);
        expect(craftRes.crupper?.finalDownhillSaddleRetentionPercent).toBe(23);
        expect(craftRes.crupper?.finalTailDockComfortPercent).toBe(13);
    });

    it("handles bench becoming non-functional after successful craft when durability falls below threshold", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_wear", "ELDER_CRUPPER_BENCH");
    ...
    it("rejects crafting when insufficient leather is provided and returns provided leathers", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_02", "ELDER_CRUPPER_BENCH");

        const failRes = AncientRunicLeatherHorseCrupperBenchEngine.craftCrupper(
            bench,
            "WARMASTER_MITHRIL_REINFORCED_CRUPPER",
            ["TEMPERED_MITHRIL_CRUPPER_DOCK_RING"]
        );

        expect(failRes.success).toBe(false);
        expect(failRes.reason).toContain("Insufficient crupper straps/dock rings");
        expect(failRes.remainingProvidedLeathers.length).toBe(1);
        expect(bench.currentDurability).toBe(95);
    });

    it("handles crupper strap misaligned failure roll consuming durability and leathers", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_03", "ELDER_CRUPPER_BENCH"); // 87% success

        const fail = AncientRunicLeatherHorseCrupperBenchEngine.craftCrupper(
            bench,
            "NOVICE_EXPEDITION_CRUPPER",
            ["TANNED_STALLION_CRUPPER_STRAP", "TANNED_STALLION_CRUPPER_STRAP", "TANNED_STALLION_CRUPPER_STRAP"],
            0.95
        );
    ...
    it("gates isFunctional in maintainBench based on DURABILITY_COST_PER_CRAFT threshold and returns clone", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_04", "ELDER_CRUPPER_BENCH");
    ...
    it("guards against null inputs and unsupported bench models", () => {
        ...
        expect(AncientRunicLeatherHorseCrupperBenchEngine.craftCrupper(invalidBench, "NOVICE_EXPEDITION_CRUPPER", ["TANNED_STALLION_CRUPPER_STRAP", "TANNED_STALLION_CRUPPER_STRAP"]).success).toBe(false);
        expect(AncientRunicLeatherHorseCrupperBenchEngine.craftCrupper(null as any, "NOVICE_EXPEDITION_CRUPPER", []).success).toBe(false);
