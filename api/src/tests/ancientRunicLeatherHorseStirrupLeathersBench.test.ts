...
describe("AncientRunicLeatherHorseStirrupLeathersBenchEngine Equestrian Stirrup Leathers Stitching Benches & Rigs", () => {
    it("crafts Celestial Void Valkyrie Sovereign Stirrup Leathers in Stirrup Leathers Sanctum achieving 100% anchor stability and returns remaining pelts", () => {
        const bench = AncientRunicLeatherHorseStirrupLeathersBenchEngine.constructBench("leather_01", "CELESTIAL_VOID_VALKYRIE_STIRRUP_LEATHERS_SANCTUM");
        expect(bench.benchType).toBe("CELESTIAL_VOID_VALKYRIE_STIRRUP_LEATHERS_SANCTUM");
        expect(bench.currentDurability).toBe(350);

        const initialLeathers = [
            "CELESTIAL_VOID_ASTRAL_STIRRUP_PELT",
            "CELESTIAL_VOID_ASTRAL_STIRRUP_PELT",
            "CELESTIAL_VOID_ASTRAL_STIRRUP_PELT"
        ] as any[];
        ...
        expect(craftRes.stirrupLeathers?.stirrupLeathersAnchorStabilityPercent).toBe(100);
        expect(craftRes.stirrupLeathers?.finalRiderWeightSuspensionPercent).toBe(100); // 84 * 1.20 = 100.8 -> clamped to 100%
        expect(craftRes.stirrupLeathers?.finalTensileAntiStretchPercent).toBe(77); // 64 * 1.20 = 76.8 -> 77%
        expect(craftRes.stirrupLeathers?.consumedLeatherCount).toBe(2);
        expect(craftRes.stirrupLeathers?.consumedLeatherType).toBe("CELESTIAL_VOID_ASTRAL_STIRRUP_PELT");
    ...
    it("verifies mid-range stability roll and sub-100% quality scaling on Elder stirrup leathers bench", () => {
        const bench = AncientRunicLeatherHorseStirrupLeathersBenchEngine.constructBench("leather_mid", "ELDER_STIRRUP_LEATHERS_BENCH");
        ...
        // finalSuspension = Math.round(24 * 0.944) = 23
        // finalAntiStretch = Math.round(14 * 0.944) = 13
        const craftRes = AncientRunicLeatherHorseStirrupLeathersBenchEngine.craftStirrupLeathers(
            bench,
            "NOVICE_EXPEDITION_STIRRUP_LEATHERS",
            ["TANNED_HEAVY_OX_STIRRUP_STRAP", "TANNED_HEAVY_OX_STIRRUP_STRAP"],
            0.1,
            0.5
        );

        expect(craftRes.success).toBe(true);
        expect(craftRes.stirrupLeathers?.stirrupLeathersAnchorStabilityPercent).toBe(36);
        expect(craftRes.stirrupLeathers?.finalRiderWeightSuspensionPercent).toBe(23);
        expect(craftRes.stirrupLeathers?.finalTensileAntiStretchPercent).toBe(13);
    });

    it("handles bench becoming non-functional after successful craft when durability falls below threshold", () => {
        const bench = AncientRunicLeatherHorseStirrupLeathersBenchEngine.constructBench("leather_wear", "ELDER_STIRRUP_LEATHERS_BENCH");
    ...
    it("rejects crafting when insufficient leather is provided and returns provided leathers", () => {
        const bench = AncientRunicLeatherHorseStirrupLeathersBenchEngine.constructBench("leather_02", "ELDER_STIRRUP_LEATHERS_BENCH");

        const failRes = AncientRunicLeatherHorseStirrupLeathersBenchEngine.craftStirrupLeathers(
            bench,
            "WARMASTER_MITHRIL_REINFORCED_STIRRUP_LEATHERS",
            ["TEMPERED_MITHRIL_STIRRUP_BUCKLE_BAR"]
        );

        expect(failRes.success).toBe(false);
        expect(failRes.reason).toContain("Insufficient stirrup straps/buckle bars");
    ...
    it("handles stirrup strap misaligned failure roll consuming durability and leathers", () => {
        const bench = AncientRunicLeatherHorseStirrupLeathersBenchEngine.constructBench("leather_03", "ELDER_STIRRUP_LEATHERS_BENCH"); // 87% success

        const fail = AncientRunicLeatherHorseStirrupLeathersBenchEngine.craftStirrupLeathers(
            bench,
            "NOVICE_EXPEDITION_STIRRUP_LEATHERS",
            ["TANNED_HEAVY_OX_STIRRUP_STRAP", "TANNED_HEAVY_OX_STIRRUP_STRAP", "TANNED_HEAVY_OX_STIRRUP_STRAP"],
            0.95
        );
    ...
    it("gates isFunctional in maintainBench based on DURABILITY_COST_PER_CRAFT threshold and returns clone", () => {
        const bench = AncientRunicLeatherHorseStirrupLeathersBenchEngine.constructBench("leather_04", "ELDER_STIRRUP_LEATHERS_BENCH");
    ...
    it("guards against null inputs and unsupported bench models", () => {
        ...
        expect(AncientRunicLeatherHorseStirrupLeathersBenchEngine.craftStirrupLeathers(invalidBench, "NOVICE_EXPEDITION_STIRRUP_LEATHERS", ["TANNED_HEAVY_OX_STIRRUP_STRAP", "TANNED_HEAVY_OX_STIRRUP_STRAP"]).success).toBe(false);
        expect(AncientRunicLeatherHorseStirrupLeathersBenchEngine.craftStirrupLeathers(null as any, "NOVICE_EXPEDITION_STIRRUP_LEATHERS", []).success).toBe(false);
