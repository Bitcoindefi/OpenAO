...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_map_state') THEN
...
CREATE INDEX IF NOT EXISTS idx_user_maps_owner ON user_maps(owner_account_id);
CREATE INDEX IF NOT EXISTS idx_user_maps_num ON user_maps(map_num);
CREATE INDEX IF NOT EXISTS idx_user_maps_state ON user_maps(state);
CREATE INDEX IF NOT EXISTS idx_user_maps_published ON user_maps(published_at DESC) WHERE state = 'published';
...
CREATE TABLE IF NOT EXISTS user_map_reviews (
    id                   UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    map_id               UUID NOT NULL REFERENCES user_maps(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS user_map_quotas (
    account_id          UUID PRIMARY KEY REFERENCES accounts(id) ON DELETE CASCADE,
